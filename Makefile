...
PKG := github.com/natemarks/asg-refresh
COMMIT := $(shell git rev-parse HEAD)
PKG_LIST := $(shell go list ${PKG}/... | grep -v /vendor/)
GO_FILES := $(shell find . -name '*.go' | grep -v /vendor/)
CDIR = $(shell pwd)
EXECUTABLES := asg-refresh
GOOS := linux darwin
GOARCH := amd64
	...
	@for o in $(GOOS); do \
	  for a in $(GOARCH); do \
        ...
        go build  -v -o build/$(COMMIT)/$${o}/$${a}/$@ \
				-ldflags="-X github.com/natemarks/asg-refresh/version.Version=${COMMIT}" ${PKG}/cmd/$@; \
	  done \
    done ; \
...
release: git-status build
	...
	@for o in $(GOOS); do \
	  for a in $(GOARCH); do \
        ...
        tar -C ./build/$(COMMIT)/$${o}/$${a} -czvf release/$(COMMIT)/asg-refresh_$(COMMIT)_$${o}_$${a}.tar.gz . ; \
	  done \
    done ; \
...
godeadcode: # run cyclomatic complexity check
	...
	deadcode -test github.com/natemarks/asg-refresh/cmd/...
...
docker-build: build ## create docker image with commit tag
	( \
	   docker build --no-cache \
       	-t asg-refresh:$(COMMIT) \
       	-t asg-refresh:latest \
       	-f Dockerfile .; \
	)
...
docker-release: docker-build ## upload the latest docker image to ECR
	( \
	   ...
	   docker tag asg-refresh:latest $(AWS_ACCOUNT_NUMBER).dkr.ecr.$(AWS_REGION).amazonaws.com/asg-refresh:latest; \
	   docker push $(AWS_ACCOUNT_NUMBER).dkr.ecr.$(AWS_REGION).amazonaws.com/asg-refresh:latest; \
	)
